# DoomsdaySSW4 游戏技术规范文档 (SPEC)

## 文档版本
- **版本号**: v1.0
- **创建日期**: 2026-1-7
- **项目名称**: DoomsdaySSW4
- **游戏类型**: 回合制探索挖矿 + 勘探船升级 + 卡牌构建
...
### 1.1 游戏核心玩法
一款以回合制探索、资源挖矿、勘探船建造和卡牌构建为核心玩法的太空勘探游戏。玩家需要管理勘探船，在地图上探索挖矿点，挖掘矿物资源，升级勘探船触发“三选一”选择升级勘探船的能力，用来提升能力。

### 1.2 核心循环
0. **初始选择阶段**（仅游戏开始时）：选择勘探船并应用还款合同效果（详见第2章）
1. **移动阶段**: 在大地图上点击探索点，勘探船直接移动到目标探索点
2. **准备阶段**: 到达探索点后，玩家角色固定在勘探船上。可以雇佣角色进行防御，雇佣的角色在怪物入侵时自动攻击。玩家可以控制1个小炮塔进行手动防御。
3. **探索阶段**: 进入探索点内部，进行挖矿和防御
4. **还款阶段**: 按照该勘探船设定的还款合同，按照一个固定的还款步骤还款，这类采用需要还款N次，每次还款限定M回合后还款的模式进行。这里随着还款次数的向后，每次还款的额度和给与的回合数都会变大。
5. **战斗/挖矿阶段**: 在探索点中挖掘矿物资源，如果是资源类的将在获得后自动按照资源的兑换比变成资金，每回合的挖掘会固定触发震动，当震动值达到最大值时触发怪物入侵防御战。防御战中，雇佣的角色自动进行防御，玩家手动控制1个小炮塔进行防御。怪物的入侵将破坏勘探船的大门，大门血量为空后，怪物将抢走资源导致资金被扣除。
6. **返回阶段**: 当完成N次还款后算作胜利，可做出选择A.离开挖矿点，B.继续挖矿（可获得新的挑战任务）
7. **功能解锁阶段**: 在勘探船中挖矿可以获得能源类物品，这些能源类物品将自动转化成勘探船的能源（经验），每增加到一个设定的值将触发三选一，对勘探船进行功能解锁。
8. **回合推进**: 点击"继续挖掘"进入下一轮循环。
...
- **合同系统**：每艘船对应一个租用合同，合同包含一个系列的还款要求，
...
### 7.1 勘探船核心概念
- **玩家角色固定在勘探船上**: 玩家角色固定在勘探船上，不能进入探索棋盘
  - **移动阶段**: 在大地图上点击探索点，勘探船直接移动到目标探索点
  - **准备阶段**: 到达探索点后，玩家角色固定在勘探船上，可以雇佣角色和选择控制炮塔
- **雇佣角色系统**: 
  - **雇佣方式**: 在准备阶段或勘探船界面中雇佣角色
  - **永久性**: 雇佣的角色是永久性的，可以一直使用
  - **自动防御**: 雇佣的角色在怪物入侵时自动进行防御
- **玩家控制炮塔**: 
  - **选择炮塔**: 在准备阶段选择1个小炮塔进行手动控制
  - **控制限制**: 同时只能控制1个炮塔，其他炮塔保持自动攻击
- **移动方式**: 勘探船作为整体在地图上移动，玩家角色始终在勘探船上

### 7.2 勘探船状态与位置规则
- **勘探船状态**: 
  - 停靠中（可进行功能解锁等操作）
  - 移动中（正在前往目标探索点的移动过程中）
  - 到达探索点（已到达探索点，可进行准备阶段操作）
  - 探索中（已进入探索点内部，正在挖矿和防御）
  - 返回中（正在返回停靠点的移动过程中）
- **位置持久化规则**: 
  ...
  - **玩家角色固定**: 玩家角色始终固定在勘探船上，不会离开
- **当前位置交互**: 
  - **在大地图上点击探索点**: 直接移动勘探船到目标探索点
  - **到达探索点后**: 进入准备阶段，可以雇佣角色和选择控制炮塔
  - **勘探船位于某探索点时**: 可以重新进入探索或进行准备阶段操作
  - **勘探船位于大地图时**: 点击其他探索点会直接开始移动
...
ProspectingShip {
    ...
    string playerExplorerId;      // 玩家角色ID（固定在勘探船上）
    List<string> hiredExplorerIds; // 已雇佣的角色ID列表（永久性）
    string playerControlledTurretId; // 玩家控制的炮塔ID（可选）
}

ExplorationState {
    string playerExplorerId;      // 玩家角色ID（固定在勘探船上，不进入棋盘）
    List<string> hiredExplorerIds; // 雇佣的角色ID列表
    string playerControlledTurretId; // 玩家控制的炮塔ID
    string explorationPointId;    // 当前探索的探索点ID
    bool isActive;                // 是否正在探索中
}
```
...
- **层数机制**: 
  ...
  - 达到最大层数后，强制结束该探索点的探索，返回地图界面，玩家可重新选择目标点
- **生成规则**: 
  - **玩家角色不进入探索棋盘**：玩家角色固定在勘探船上，不会出现在探索棋盘上
  - **雇佣角色不进入探索棋盘**：雇佣的角色在防御战中自动出现在防御位置（探索棋盘外围），不会出现在探索棋盘上
  - **棋盘内容放置（虫族与矿物）**：对棋盘上所有格子按照格子编号由小到大遍历：
...
### 8.3 探索棋盘规则
- **空间结构**: 探索棋盘为4格宽 × 6格高（共24个格子）
- **格子编号规则**: 
  - 格子编号从左上角第1个格子开始作为编号1，先向右编号（1, 2, 3, 4），再向下到下一行继续编号（5, 6, 7, 8），以此类推直到最右下方的格子编号为24
  - 编号公式：`格子编号 = (行号 - 1) × 4 + 列号`（行号和列号从1开始）
  - 格子编号用于战斗系统的攻击顺序判定（详见8.1战斗规则）
- **角色占用**: 玩家角色和雇佣角色不占用探索棋盘格子，它们固定在勘探船上或防御位置
- **虫族生成**: 根据配置表设定的虫族类型和数量
- **矿物生成**: 原始矿物资源，玩家在勘探船上进行挖矿操作
- **基础产出**: 矿物有默认基础产出
- **产出提升**: 通过玩家角色技能、天赋、科技树提升
- **回合与体力规则**: 
  - 探索棋盘内不再进行大地图移动
...
- **探索完成条件**: 
  - 玩家角色固定在勘探船上，探索完成条件改为：完成当前层的挖矿操作或达到最大层数
  - 玩家也可以主动选择退出当前层（返回地图界面）
  - 如果勘探船船体血量降为0（在防御战中），则探索失败，游戏失败
- **层数规则**: 
  - 每次进入探索点均从第1层开始
...
- **棋盘刷新规则**: 
  - **每次进入新的一层探索棋盘时，必须对所有单位进行随机位置刷新**：
    - 玩家角色和雇佣角色：不进入探索棋盘，保持在勘探船上或防御位置
    - 虫族位置：根据"棋盘出现内容"配置，按权重随机生成虫族并随机放置在棋盘格子上
    - 矿物位置：根据"棋盘出现内容"配置，按权重随机生成矿物并随机放置在棋盘格子上
  - 同一探索点的不同层之间，虫族、矿物的位置必须完全重新随机化，不能保留上一层的位置
...
ExplorationSpace {
    ...
    bool playerOnShip;            // 玩家是否在勘探船上（固定为true）
}
```
...
雇佣角色系统允许玩家雇佣角色进行防御。雇佣的角色是永久性的，可以一直使用。当怪物入侵触发时，雇佣的角色会自动出现在防御位置并进行自动防御。

**核心设计理念**：
- **永久性雇佣**：雇佣的角色是永久性的，一旦雇佣就可以一直使用
- **自动防御**：雇佣的角色在怪物入侵时自动进行防御，无需玩家手动控制
- **防御位置**：雇佣的角色在防御战中自动出现在探索棋盘外围的防御位置
- **雇佣成本**：雇佣角色需要消耗资源，成本在角色配置表中定义
...
- **雇佣方式**：
  ...
  - 选择角色后，消耗资源完成雇佣
- **雇佣成本**：
  - 每个角色有独立的雇佣成本（在 `ExplorerConfig.雇佣成本资源列表` 中配置）
  - 成本格式：`资源ID_数量|资源ID_数量`
  - 雇佣时从勘探船仓库中扣除对应资源
- **雇佣状态**：
  - **未雇佣**：角色未解锁，需要消耗资源雇佣
  - **已雇佣**：角色已解锁，可以参与防御战
- **雇佣限制**：
  - 每个角色只能雇佣一次（永久性）
  - 雇佣后角色永久可用，不会因为探索结束而失效

#### 8.7.3 雇佣角色数据结构

```csharp
HiredExplorer {
    string explorerId;        // 角色ID
    bool isHired;            // 是否已雇佣
    Map<string, int> hireCost; // 雇佣成本（资源ID -> 数量）
    DefenseRole role;        // 防御角色类型
    Vector2Int defensePosition; // 防御位置（探索棋盘外围坐标）
}

DefenseRole {
    AutoDefender,    // 自动防御者（在防御战中自动攻击）
    // 未来可扩展其他角色类型
}

ProspectingShip {
    // ... 现有字段 ...
    List<string> hiredExplorerIds; // 已雇佣的角色ID列表
}
```
...
- **防御位置**：
  - 雇佣的角色在防御战中自动出现在探索棋盘外围的防御位置
  - 防御位置在探索棋盘外围（与防御设施位置类似）
  - 位置分配：系统自动为每个雇佣的角色分配一个防御位置
- **自动防御**：
  - 当怪物入侵触发时，所有已雇佣的角色自动出现在防御位置
  - 雇佣的角色按照攻击规则自动攻击范围内的怪物
  - 攻击顺序和规则与现有战斗系统类似
  - 攻击范围：默认攻击范围为相邻格子（8方向，距离1格），可通过角色属性或科技树扩展
- **防御状态**：
  - 雇佣的角色在防御战中可以受到伤害
  - 如果雇佣的角色血量降为0，该角色在本次防御战中无法继续战斗
  - 防御战结束后，雇佣的角色状态恢复（血量不自动恢复，需要通过医疗包等治疗）

#### 8.7.5 雇佣角色配置表扩展

在 `ExplorerConfig` 中新增字段：

| 字段名 | 类型 | 说明 | 示例 |
|--------|------|------|------|
| 雇佣成本资源列表 | string | 雇佣消耗的资源列表（格式：资源ID_数量\|资源ID_数量） | "resource_metal_10\|resource_food_5" |
| 是否可雇佣 | boolean | 该角色是否可以被雇佣 | true |
| 防御角色类型 | string | 防御角色类型（如"AutoDefender"） | "AutoDefender" |
...
- **雇佣角色管理面板**：
  - 位置：在准备阶段界面或勘探船界面中显示
  - 显示内容：
    - 可雇佣的角色列表（显示角色信息、雇佣成本）
    - 已雇佣的角色列表（显示角色状态、防御位置）
  - 交互功能：
    - 雇佣操作：选择角色，确认消耗资源，完成雇佣
    - 查看详情：查看角色的详细信息和防御能力
- **防御战中的显示**：
  - 在防御战界面中显示雇佣角色的位置和状态
  - 显示雇佣角色的自动攻击动画和效果
...
3. **角色配置表 (ExplorerConfig)**
   ...
   - 初始科技树解锁项
   - 末日前身份标签
   - **雇佣成本资源列表**（hireCostResources，格式：`资源ID_数量|资源ID_数量`）- 雇佣该角色需要消耗的资源
   - **是否可雇佣**（isHirable，boolean）- 该角色是否可以被雇佣
   - **防御角色类型**（defenseRole，string）- 防御角色类型（如"AutoDefender"）
...
- **地图与探索棋盘规模**  
  ...
  - 单个探索棋盘固定为 4×6（24 个格子），后续如需扩展需重新评估性能与可玩性。
- **单位数量上限（Demo 阶段）**  
...
怪物入侵系统是游戏的核心防御机制，当玩家在挖掘点挖掘时，震动值会累积，达到最大值时触发怪物入侵。入侵怪物会攻击勘探船和防御设施，玩家需要依靠雇佣角色、防御设施和手动控制的炮塔进行防御。

**核心设计理念**：
- **震动值累积**：每回合自动增加，道具触发时也会增加
- **即时制战斗**：入侵战斗采用即时制，怪物和防御单位自动战斗
- **雇佣角色自动防御**：雇佣的角色在防御战中自动出现在防御位置并进行自动攻击
- **玩家手动控制炮塔**：玩家可以手动控制1个小炮塔进行防御
- **防御设施自动防御**：其他防御设施自动攻击范围内的怪物
- **游戏失败条件**：船体血量降为0时游戏失败
...
- **战斗流程**：
  1. 更新怪物目标（优先攻击设施）
  2. **雇佣角色自动攻击阶段**：所有已雇佣的角色自动攻击范围内的怪物
  3. **玩家控制炮塔攻击阶段**：玩家手动控制的炮塔攻击玩家选择的目标
  4. **其他防御设施自动攻击阶段**：其他防御设施自动攻击范围内的怪物
  5. 怪物攻击设施、雇佣角色或船体
  6. 检查胜利/失败条件
...
- **攻击速度**：每秒攻击次数
- **等级**：设施可以升级，提升属性
- **是否可手动控制**：标记该设施是否可以被玩家手动控制（在 `DefenseFacilityConfig.是否可手动控制` 中配置）
...
- 防御设施放置在探索棋盘外围位置：
  ...
  - 左边缘：x = -1, y = 0-3
  - 右边缘：x = 6, y = 0-3
- 雇佣的角色也放置在探索棋盘外围的防御位置（与防御设施位置类似）
...
#### 24.5.5 玩家控制炮塔机制
- **选择炮塔**：
  - 在准备阶段或防御战开始前，玩家可以选择1个小炮塔进行手动控制
  - 只有标记为"可手动控制"的炮台设施才能被玩家控制
  - 同时只能控制1个炮塔，其他炮塔保持自动攻击
- **控制方式**：
  - 玩家可以点击目标怪物，炮塔会攻击该目标
  - 如果目标在炮塔攻击范围内，炮塔立即攻击
  - 如果目标不在攻击范围内，炮塔会尝试攻击范围内的其他目标（自动模式）
- **控制限制**：
  - 同时只能控制1个炮塔
  - 玩家控制的炮塔优先攻击玩家选择的目标
  - 如果玩家选择的目标死亡或超出范围，炮塔自动切换到自动攻击模式
- **控制切换**：
  - 在防御战中，玩家可以切换控制不同的炮塔（如果存在多个可控制的炮塔）
  - 切换控制时，原炮塔恢复自动攻击模式
...
- `建造消耗资源列表`: string
- `升级消耗资源列表`: string
- `是否可手动控制`: boolean - 标记该设施是否可以被玩家手动控制
- `手动控制优先级`: number - 用于排序显示可控制的设施列表（数字越小优先级越高）
...
interface InvasionState {
  ...
  hiredDefenders: Map<string, HiredExplorer>; // 雇佣的防御角色列表
  playerControlledTurretId: string | null; // 玩家手动控制的炮塔ID
  defenderPositions: Map<string, { x: number; y: number }>; // 防御角色位置（角色ID -> 位置）
  startTime: number; // 入侵开始时间
}
```

#### 24.9.5 新增 HiredExplorer（防御战中的雇佣角色）
```typescript
interface HiredExplorer {
  explorerId: string; // 角色ID
  explorer: Explorer; // 角色实例
  defensePosition: { x: number; y: number }; // 防御位置（探索棋盘外围坐标）
  lastAttackTime: number; // 上次攻击时间
  role: 'AutoDefender'; // 防御角色类型
}
```

#### 24.9.6 新增 PlayerControlledTurret
```typescript
interface PlayerControlledTurret {
  facilityId: string; // 炮塔设施ID
  isPlayerControlled: boolean; // 是否由玩家控制
  targetMonsterId: string | null; // 当前攻击目标（玩家选择）
  lastAttackTime: number; // 上次攻击时间
}
```
...
- **功能**：
  ...
  - 选择建造位置
  - 升级现有设施
  - 选择玩家控制的炮塔（标记为"可手动控制"的设施）

#### 24.10.3 雇佣角色管理面板
- **位置**：在准备阶段界面或勘探船界面中显示
- **功能**：
  - 显示可雇佣的角色列表（显示角色信息、雇佣成本）
  - 显示已雇佣的角色列表（显示角色状态）
  - 雇佣操作：选择角色，确认消耗资源，完成雇佣
  - 查看角色详情：查看角色的详细信息和防御能力
